...
namespace RoguelikeSouls.Installation
{
    ...
    class WeaponStats
    {
        ...
        public Attack GetAttackParam(GameParamHandler gParam, int behaviorSubId)
        {
            ...
            if (behavior.ReferenceType != 0)
                throw new ArgumentException(
                    $"Behavior param {behaviorSubId} has reference type {behavior.ReferenceType}, not 0 (Attack).");
        ...
        public Bullet GetBulletParam(GameParamHandler gParam, int behaviorSubId)
        {
            ...
            if (behavior.ReferenceType != 1)
                throw new ArgumentException(
                    $"Behavior param {behaviorSubId} has reference type {behavior.ReferenceType}, not 1 (Bullet).");
    ...
    class WeaponGenerator
    {
        ...
        public static Dictionary<int, string> MeleeBehaviorSubNames = new Dictionary<int, string>()
        {
            ...
            {
                130, "1H strong attack 2 (special)"
            }, // probably 3310 (param exists for most special weapons but never called in TAE)
        ...
        List<int> LegendaryShieldSpecials { get; } =
            new List<int>() { 92, 94, 126 }; // Crystal Ring, Havel's, Bonewheel

        Behavior BulletBehaviorTemplate
        {
            get => Mod.VanillaGPARAM.BehaviorsPC[103603800];
        }
        ...
        public int GetRandomWeaponID(bool generic, bool legendary, bool abyssal,
            (Upgrade upgrade, double odds)[] upgradeOptions)
        {
        ...
        void CreateStartingWeapons()
        {
            ...
            Weapon largeLeatherShield =
                StartingWeaponSetup(1402000, FixedWeapons["Large Leather Shield"], ref behaviorVarId);
            ...
            Weapon smallLeatherShield =
                StartingWeaponSetup(1403000, FixedWeapons["Small Leather Shield"], ref behaviorVarId);
...
#endif
            Mod.GPARAM.Weapons.DeleteRowWhereID(id =>
                id >= lastWeaponParam && id < 2000000 && id != VanillaWeapons["Fists"].ParamId);
...
#endif
            Mod.GPARAM.BehaviorsPC.DeleteRowWhereID(id =>
                id >= lastBehaviorParam && !(104201000 <= id && id < 104202000));
        ...
        void CreateWeaponFromFunction(Action<int, int, bool, bool> createWeaponFunc, ref int weaponIndex,
            bool isAbyssal = false)
        {
        ...
        public void CreateMeleeWeapon(int weaponParamId, int behaviorVarId, bool isLegendary = false,
            bool isAbyssal = false)
        {
            ...
            LegendaryMeleeEffects legendaryEffects =
                isLegendary ? new LegendaryMeleeEffects(isAbyssal, specialWeaponStats != null, Rand) : null;
            double staminaCostMultiplier = legendaryEffects != null ? legendaryEffects.GetStaminaMultiplier() : 1.0;

            Weapon newWeapon = BasicWeaponSetup(weaponParamId, behaviorVarId, legendaryEffects, modelWeaponStats,
                baseWeaponStats, specialWeaponStats);
            ...
            if (!SkipBehaviors)
            {
                for (int subId = 0; subId < 1000; subId++)
                {
                    ...
                    Attack newAttack = BasicBehaviorAttackSetup(behaviorVarId, subId, behaviorName, modelWeaponStats,
                        baseWeaponStats, specialWeaponStats, staminaCostMultiplier);
                    if (newAttack != null && !MeleeBehaviorSubNames.ContainsKey(subId))
                        throw new Exception(
                            $"Unrecognized melee attack sub ID: {subId} ({baseWeaponStats.NameID} | {specialWeaponStats?.NameID}");
            ...
            if (isLegendary)
            {
                ...
                TAE specialAnimationTAE = specialWeaponStats != null
                    ? Mod.Player.GetTAE(specialWeaponStats.SpecialAttackAnimationCategory)
                    : null;
        ...
        public void CreateCatalyst(int weaponParamId, int behaviorVarId, bool isLegendary = false,
            bool isAbyssal = false)
        {
            isLegendary |= isAbyssal;
            LegendaryCatalystEffects legendaryEffects =
                isLegendary ? new LegendaryCatalystEffects(isAbyssal, Rand) : null;
            ...
            if (!SkipBehaviors)
            {
                for (int subId = 0; subId < 1000; subId++)
                {
                    ...
                    BasicBehaviorAttackSetup(behaviorVarId, subId, $"{newWeapon.Name} ({subId})", modelWeaponStats,
                        baseWeaponStats, specialWeaponStats);
                }
            }
        ...
        public void CreateTalisman(int weaponParamId, int behaviorVarId, bool isLegendary = false,
            bool isAbyssal = false)
        {
            isLegendary |= isAbyssal;
            LegendaryTalismanEffects legendaryEffects =
                isLegendary ? new LegendaryTalismanEffects(isAbyssal, Rand) : null;
            ...
            WeaponStats
                baseWeaponStats =
                    VanillaWeapons
                        ["Fists"]; // TODO: isLegendary ? VanillaWeapons["Fists"] : GetRandomVanillaWeapon("Talisman");
            WeaponStats specialWeaponStats =
                (legendaryEffects != null && legendaryEffects.EffectPoints["DragonBoneFistSpecial"] == 1)
                    ? VanillaWeapons["DragonBoneFist"]
                    : null;
            ...
            if (!SkipBehaviors)
            {
                for (int subId = 0; subId < 1000; subId++)
                {
                    if (subId >= 800 && (baseWeaponStats.WeaponClass == "Melee" ||
                                         (specialWeaponStats != null && specialWeaponStats.WeaponClass == "Melee")))
        ...
        {
            isLegendary |= isAbyssal;
            LegendaryBowCrossbowEffects legendaryEffects =
                isLegendary ? new LegendaryBowCrossbowEffects(isAbyssal, isCrossbow: false, Rand) : null;
            ...
            Weapon newBow = BasicWeaponSetup(weaponParamId, behaviorVarId, legendaryEffects, modelWeaponStats,
                baseWeaponStats, specialWeaponStats);
            newBow.BowRangeChangePercentage =
                specialWeaponStats.GetWeaponParam(Mod.VanillaGPARAM).BowRangeChangePercentage;
                ...
                for (int subId = 0; subId < 1000; subId++)
                {
                    if (subId == 300)
                    {
                        ...
                        Behavior sourceBehavior =
                            (specialWeaponStats ?? baseWeaponStats).GetBehaviorParam(Mod.VanillaGPARAM, subId);
                        Behavior newBehavior = Mod.GPARAM.BehaviorsPC.CopyRow(sourceBehavior,
                            100000000 + 1000 * behaviorVarId + subId);
                    ...
                    {
                        BasicBehaviorAttackSetup(behaviorVarId, subId, $"{newBow.Name} ({subId})", modelWeaponStats,
                            baseWeaponStats, specialWeaponStats, staminaCostMultiplier);
                    }
                }
            }
        ...
        public void CreateCrossbow(int weaponParamId, int behaviorVarId, bool isLegendary = false,
            bool isAbyssal = false)
        {
            isLegendary |= isAbyssal;
            LegendaryBowCrossbowEffects legendaryEffects =
                isLegendary ? new LegendaryBowCrossbowEffects(isAbyssal, isCrossbow: true, Rand) : null;
            ...
            Weapon newCrossbow = BasicWeaponSetup(weaponParamId, behaviorVarId, legendaryEffects, modelWeaponStats,
                baseWeaponStats, specialWeaponStats);
            newCrossbow.BowRangeChangePercentage =
                specialWeaponStats.GetWeaponParam(Mod.VanillaGPARAM).BowRangeChangePercentage;
            double staminaCostMultiplier = legendaryEffects != null ? legendaryEffects.GetStaminaMultiplier() : 1.0;
            ...
            if (!SkipBehaviors)
            {
                for (int subId = 0; subId < 1000; subId++)
                {
                    if (subId == 0 || subId == 300 || subId == 400)
                    {
                        ...
                        newBehavior.StaminaCost = (int)(newBehavior.StaminaCost * staminaCostMultiplier);
                        newBehavior.Category =
                            (byte)(subId == 400 ? 2 : 1); // 1 (right hand) for 0 and 300, 2 (left hand) for 400
                    }
                    else
                    {
                        BasicBehaviorAttackSetup(behaviorVarId, subId, $"{newCrossbow.Name} ({subId})",
                            modelWeaponStats, baseWeaponStats, specialWeaponStats);
                    }
                }
            }
        ...
        public void CreateShield(int weaponParamId, int behaviorVarId, bool isLegendary = false, bool isAbyssal = false)
        {
            ...
            if (legendaryEffects != null)
            {
                ...
                else if (legendaryEffects.EffectPoints["SpecialSuperBash"] == 1)
                {
                    ...
                    var bashingShieldChoices = new List<KeyValuePair<string, WeaponStats>>(VanillaWeapons.Where(
                        kv => kv.Value.WeaponClass == "Shield" &&
                              !LegendaryShieldSpecials.Contains(kv.Value.SpecialAttackAnimationCategory) &&
                              (kv.Value.WeaponSubclass == "Greatshield" ||
                               kv.Value.SpecialAttackAnimationCategory == 93)));
                    specialWeaponStats = bashingShieldChoices.GetRandomElement(Rand).Value;
                }
                else if (modelWeaponStats.WeaponSubclass == "Greatshield" &&
                         legendaryEffects.EffectPoints["Light"] == 1)
                {
            ...
            if (specialWeaponStats == null && Rand.NextDouble() < ShieldSpecialAnimationOdds)
            {
                ...
                var nonSpecialShieldChoices = new List<KeyValuePair<string, WeaponStats>>(VanillaWeapons.Where(
                    kv => kv.Value.WeaponClass == "Shield" && kv.Value.WeaponSubclass == modelWeaponStats.WeaponSubclass
                                                           && !LegendaryShieldSpecials.Contains(
                                                               kv.Value.SpecialAttackAnimationCategory)));
                specialWeaponStats = nonSpecialShieldChoices.GetRandomElement(Rand).Value;
            }

            Weapon newWeapon = BasicWeaponSetup(weaponParamId, behaviorVarId, legendaryEffects, modelWeaponStats,
                baseWeaponStats, specialWeaponStats);
            newWeapon.PhysicalGuardPercentage -=
                10.0f; // Default physical guard is reduced by 10, so only legendary shields can reach 100.
            double staminaCostMultiplier = legendaryEffects != null ? legendaryEffects.GetStaminaMultiplier() : 1.0;
                ...
                for (int subId = 0; subId < 1000; subId++)
                {
                    if (subId == 800)
                    {
                        if (specialWeaponStats != null && specialWeaponStats.SpecialAttackAnimationCategory == 92)
                        {
                            ...
                            Behavior newBehavior = Mod.GPARAM.BehaviorsPC.CopyRow(sourceBehavior,
                                100000000 + 1000 * behaviorVarId + 800);
                            newBehavior.VariationID = behaviorVarId;
            ...
            int upgradeOffset =
                (specialWeaponStats != null && specialWeaponStats.SpecialAttackAnimationCategory.In(93, 126))
                    ? 0
                    : 5000;
            // Generate upgrade weapon param IDs (excludes some categories).
        ...
        )
        {
            ...
            string weaponName = NameGenerator.GetRandomName(out _, modelWeaponStats.WeaponSubclass,
                legendaryEffects != null, exact: false);
            string legendaryDescription = legendaryEffects != null ? legendaryEffects.GetEffectDescription() : "";
            string weaponDesc =
                DescriptionGenerator.GetRandomDescription(extraParagraph: legendaryDescription, exact: false);
            ...
            Mod.GPARAM.BehaviorsPC.DeleteRowRange(100000000 + 1000 * behaviorVarId,
                100000000 + 1000 * (behaviorVarId + 1));
            ...
            Weapon newWeapon =
                Mod.GPARAM.Weapons.CopyRow(modelWeaponStats.GetWeaponParam(Mod.VanillaGPARAM), weaponParamId);
            CleanWeaponParam(newWeapon,
                modelWeaponStats.WeaponSubclass); // removes default elements, special effects, etc.
...
#if DEBUG
            Console.WriteLine(
                $"Creating starting weapon {newWeaponParamID} (base behavior ID {newBehaviorBaseID}, base attack ID {newAttackBaseID})");
#endif
            ...
            for (int i = 0; i < 1000; i++)
            {
                ...
                if (Mod.VanillaGPARAM.BehaviorsPC.Keys.Contains(oldBehaviorParamID))
                {
                    Behavior newBehavior =
                        Mod.GPARAM.BehaviorsPC.CopyRow(Mod.VanillaGPARAM.BehaviorsPC[oldBehaviorParamID],
                            newBehaviorBaseID + i);
                    ...
                    if (Mod.VanillaGPARAM.AttacksPC.Keys.Contains(oldAttackParamID))
                        Mod.GPARAM.AttacksPC.CopyRow(Mod.VanillaGPARAM.AttacksPC[oldAttackParamID],
                            newAttackBaseID + i);
                }
            }
        ...
        void CreateMagicVariant(Weapon weapon, int upgradeIdOffset = 0)
        {
            ...
            magicWeapon.Name = "法术" + weapon.Name;
            magicWeapon.Summary = weapon.Summary;
            magicWeapon.Description = weapon.Description;
            magicWeapon.IntelligenceScaling += 0.35f *
                                               (magicWeapon.StrengthScaling + magicWeapon.DexterityScaling +
                                                magicWeapon.FaithScaling);
        ...
        void CreateEnchantedVariant(Weapon weapon, int upgradeIdOffset = 0)
        {
            ...
            enchantedWeapon.Name = "魔力" + weapon.Name;
            enchantedWeapon.Summary = weapon.Summary;
            enchantedWeapon.Description = weapon.Description;
            enchantedWeapon.SpecialEffectOnHit0 =
                SpEffectGenerator.Effects["Enchanted (MOD HIT FLAG)"]; // Recharges spell usages (through mod hook).
            enchantedWeapon.IntelligenceScaling += 0.98f * 0.35f * (enchantedWeapon.StrengthScaling +
                                                                    enchantedWeapon.DexterityScaling +
                                                                    enchantedWeapon.FaithScaling);
        ...
        void CreateDivineVariant(Weapon weapon, int upgradeIdOffset = 0)
        {
            ...
            divineWeapon.FaithScaling += 0.35f * (divineWeapon.StrengthScaling + divineWeapon.DexterityScaling +
                                                  divineWeapon.IntelligenceScaling);
            ...
            if (weapon.WeakToDivineDamageMultiplier != 0.0f)
                divineWeapon.WeakToDivineDamageMultiplier =
                    1.2f; // note increase from vanilla 1.1 (unless Abyssal curse).
        ...
        void CreateDireVariant(Weapon weapon, int upgradeIdOffset = 0)
        {
            ...
            direWeapon.Name = "邪教" + weapon.Name;
        ...
        void CreateDraconicVariant(Weapon weapon, int upgradeIdOffset = 0)
        {
            ...
            draconicWeapon.Name = "龙族" + weapon.Name;
            ...
            draconicWeapon.EquippedSpecialEffect0 =
                SpEffectGenerator.Effects["Draconic Burn (Apply)"]; // Enemy Burn (via attack trigger)
        ...
        Attack BasicBehaviorAttackSetup(int newBehaviorVarId, int behaviorSubId, string behaviorName,
            WeaponStats modelWeaponStats, WeaponStats baseWeaponStats, WeaponStats specialWeaponStats,
            double staminaCostMultiplier = 1.0)
        {
            ...
            // except that Attack hitbox information is taken from model weapon.
            Behavior sourceBehavior =
                (specialWeaponStats ?? baseWeaponStats).GetBehaviorParam(Mod.VanillaGPARAM, behaviorSubId);
            if (sourceBehavior == null)
                return null;
            Attack sourceAttack =
                (specialWeaponStats ?? baseWeaponStats).GetAttackParam(Mod.VanillaGPARAM, behaviorSubId);
            int newAttackId = 1000 * newBehaviorVarId + behaviorSubId;
            Behavior newBehavior =
                Mod.GPARAM.BehaviorsPC.CopyRow(sourceBehavior, 100000000 + 1000 * newBehaviorVarId + behaviorSubId);
        ...
        void RandomizeRequiredStats(Weapon weapon, string weaponSubclass, double randomRange)
        {
            ...
            weapon.RequiredStrength = strength > 8
                ? (byte)(strength - randomRange + Rand.NextDouble() * 2 * randomRange)
                : strength;
            weapon.RequiredDexterity = dexterity > 8
                ? (byte)(dexterity - randomRange + Rand.NextDouble() * 2 * randomRange)
                : dexterity;
            weapon.RequiredIntelligence = intelligence > 8
                ? (byte)(intelligence - randomRange + Rand.NextDouble() * 2 * randomRange)
                : intelligence;
            weapon.RequiredFaith =
                faith > 8 ? (byte)(faith - randomRange + Rand.NextDouble() * 2 * randomRange) : faith;
        }
        ...
        void RandomizeCombatValues(Weapon weapon, Weapon modelWeapon, Weapon baseWeapon)
        {
            ...
            weapon.BasePhysicalDamage =
                (ushort)(w * baseWeapon.BasePhysicalDamage + (1.0 - w) * modelWeapon.BasePhysicalDamage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BaseStaminaDamage =
                (ushort)(w * baseWeapon.BaseStaminaDamage + (1.0 - w) * modelWeapon.BaseStaminaDamage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BasePoiseDamage = (ushort)(w * baseWeapon.BasePoiseDamage + (1.0 - w) * modelWeapon.BasePoiseDamage);

            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.PhysicalGuardPercentage = (ushort)(w * baseWeapon.PhysicalGuardPercentage +
                                                      (1.0 - w) * modelWeapon.PhysicalGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.MagicGuardPercentage =
                (ushort)(w * baseWeapon.MagicGuardPercentage + (1.0 - w) * modelWeapon.MagicGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.FireGuardPercentage =
                (ushort)(w * baseWeapon.FireGuardPercentage + (1.0 - w) * modelWeapon.FireGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.LightningGuardPercentage = (ushort)(w * baseWeapon.LightningGuardPercentage +
                                                       (1.0 - w) * modelWeapon.LightningGuardPercentage);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.GuardStaminaDefense =
                (short)(w * baseWeapon.GuardStaminaDefense + (1.0 - w) * modelWeapon.GuardStaminaDefense);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.PoisonDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.PoisonDamageReductionWhenGuarding +
                                                               (1.0 - w) * modelWeapon
                                                                   .PoisonDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.ToxicDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.ToxicDamageReductionWhenGuarding +
                                                              (1.0 - w) * modelWeapon.ToxicDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.BleedDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.BleedDamageReductionWhenGuarding +
                                                              (1.0 - w) * modelWeapon.BleedDamageReductionWhenGuarding);
            w = Clamp(GetRandNormal(0.7, 0.15));
            weapon.CurseDamageReductionWhenGuarding = (sbyte)(w * baseWeapon.CurseDamageReductionWhenGuarding +
                                                              (1.0 - w) * modelWeapon.CurseDamageReductionWhenGuarding);
        }
        ...
        void CleanWeaponParam(Weapon weapon, string weaponSubclass, bool isStartingWeapon = false)
        {
            ...
            weapon.WeaponUpgradeID =
                0; // All base weapons use default upgrade path (and only level 0, so effectively none).
            ...
            if (!isStartingWeapon)
            {
                ...
                weapon.EquippedSpecialEffect1 =
                    -1; // Reserved for positive passive legendary effect (e.g. damage boost).
                weapon.EquippedSpecialEffect2 =
                    -1; // Reserved for negative passive abyssal effect (e.g. slow movement).

                // Intrinsic elemental damage is zeroed out, but a third of their base damage is added to physical.
                weapon.BasePhysicalDamage += (ushort)(weapon.BaseMagicDamage / 3 + weapon.BaseFireDamage / 3 +
                                                      weapon.BaseLightningDamage / 3);
